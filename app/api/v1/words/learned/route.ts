...
import { getLearnedWordIds, markWordsAsLearned } from "@/services/word.service";
 ...
 *   post:
 *     summary: Bulk mark words as learned
 *     description: Marks the given words as learned for the current authenticated user in a single request.
 *     tags:
 *       - Words
 *     requestBody:
 *       required: true
 ...
 *                 items:
 *                   type: integer
 *                 description: Word ids to mark as learned
 *     responses:
 *       200:
 *         description: Words marked as learned successfully
 *       400:
 *         description: Invalid request body
 *       401:
 *         description: Unauthorized
 *       500:
 *         description: Failed to mark words as learned
 */
...
export async function POST(request: NextRequest) {
    ...
    const result = await markWordsAsLearned(
        session.user.id,
        wordIds as number[]
    );
