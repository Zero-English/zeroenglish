...
// Grammar questions carry a difficulty (EASY/MEDIUM/HARD) rather than a CEFR
// level, but QuizResults only stores Levels. Map each difficulty to the level
// band it represents so grammar practice results can live in the same table.
...
function newClientId(): string {
  return typeof crypto !== "undefined" && "randomUUID" in crypto
    ? crypto.randomUUID()
    : `gq-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
}
...
function toPlayQuestion(q: GrammarQuizQuestion): PlayQuestion {
  return {
    id: q.id,
...
async function saveGrammarResultToDb(args: {
  userId: number | null;
  clientId: string;
  quizType: string;
...
export function GrammarPracticeSession({
  ...
  const questionsRef = useRef<GrammarQuizQuestion[] | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(
      `/api/v1/quiz/by-type?quizType=${encodeURIComponent(topic.name)}&limit=150`
    )
      .then((res) => res.json())
      .then((json) => {
        if (cancelled) return;
