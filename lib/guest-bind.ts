 ...
 * results, learned words, bookmarked words) to the database for the user that
 * is currently signed in via NextAuth. Must be called BEFORE the auth store
 * identity is switched away from the guest, so the in-memory guest entries are
 * still the ones the quiz-history store serves.
 */
export async function syncGuestDataToServer(
  scope: string
): Promise<GuestSyncResult> {
  const result: GuestSyncResult = { quiz: 0, learned: 0, bookmarked: 0, failed: 0 };
